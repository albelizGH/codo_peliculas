...
async function dibujarTendencias(pagina) {
	...
	let response = await fetch(
		`https://api.themoviedb.org/3/discover/movie?language=es-MX&page=${pagina}&primary_release_date=2024&region=AR%2CUS%2CMX%2CES&sort_by=popularity.desc&year=2024&api_key=${API_KEY}`
	);
	let data = await response.json();
	let resultados = data.results;
	let estructuraHtml = resultados.map((pelicula) => {
		return `
...
async function dibujarAclamadas() {
	...
	const URL_IMAGEN_BASICA = "https://image.tmdb.org/t/p/w300";
	let response = await fetch(
		`https://api.themoviedb.org/3/movie/top_rated?region=AR%2CUS%2CCA%2CGB%2CES%2CMX?language=es&page=${pagina}&api_key=${API_KEY}`
	);
	let data = await response.json();
	let resultados = data.results;
	resultados.sort(
